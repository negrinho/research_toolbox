...
def get_lithium_resource_availability(servername, username, password=None,
        abort_if_any_node_unavailable=True, nodes_to_query=None):
...
def run_on_lithium_node(bash_command, node, servername, username, password=None,
    ...
    return run_on_server(cmd, **tb_ut.retrieve_values(locals(),
...
class LithiumRunner:
    ...
    def register(self, bash_command, num_cpus=1, num_gpus=0,
        ...
        self.jobs.append(tb_ut.retrieve_values(locals(),
    ...
    def run(self, run_only_if_enough_resources_for_all=True):
        args = tb_ut.retrieve_values(vars(self), ['servername', 'username', 'password'])
        ...
        for x in self.jobs:
            ...
            if x['require_gpu_types'] is not None:
                req_gpu_nodes = tb_ut.flatten(
                    tb_ut.retrieve_values(g, x['require_gpu_types']))
            else:
        ...
        for x, c in zip(self.jobs, run_cfgs):
            if c is None:
                ...
                out = run_on_lithium_node(**tb_ut.merge_dicts([
                    tb_ut.retrieve_values(vars(self),
                        ['servername', 'username', 'password']),
                    tb_ut.retrieve_values(x,
                        ['bash_command', 'folderpath',
                        'wait_for_output', 'run_on_head_node']),
                    tb_ut.retrieve_values(c,
...
def run_on_matrix(bash_command, servername, username, password=None,
    ...
    return run_on_server(remote_cmd, **tb_ut.retrieve_values(
